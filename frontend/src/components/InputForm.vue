...
export default {
  ...
  data: () => ({
    ...
    submissionErrors: null as SubmissionError | null,
    draftReport: null,
  ...
  methods: {
    ...
    setErrorAlert(submissionErrors: SubmissionError | null) {
    ...
    toSubmissionError(
      error: string | { message: string } | SubmissionError,
    ): SubmissionError {
      if (typeof error == 'string') {
        ...
        return this.toSubmissionError(error?.response?.data?.error);
        ...
        error.hasOwnProperty('bodyErrors') &&
        error.hasOwnProperty('rowErrors') &&
        error.hasOwnProperty('generalErrors')
      ) {
    ...
    async submit() {
      ...
      try {
        ...
        this.onSubmitComplete(this.toSubmissionError(error));
        return;
      }
      ...
      try {
        ...
        const submissionError = error.response.data?.error;
        this.onSubmitComplete(this.toSubmissionError(error)); //TODO check the Axios error is handled
      }
    },
    /* This function should be called either after a submission was
    successful, or after it errors out. */
    onSubmitComplete(submissionErrors: SubmissionError | null) {
      if (submissionErrors) {
