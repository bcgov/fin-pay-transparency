...
describe("getReportAndCalculations", () => {
  describe("when a valid report id is provided", () => {
    it("returns an object containing both the report and the values of its calculations", async () => {
      ...
      (prisma.pay_transparency_report.findFirst as jest.Mock).mockResolvedValue(mockReportInDB);
      (prisma.pay_transparency_calculated_data.findMany as jest.Mock).mockResolvedValue(mockCalculatedDatasInDB);

      const reportAndCalculations: ReportAndCalculations = await reportService.getReportAndCalculations(mockReq, mockReportId);

      expect(prisma.pay_transparency_company.findFirst).toHaveBeenCalledTimes(1);
      expect(prisma.pay_transparency_report.findFirst).toHaveBeenCalledTimes(1);
      expect(prisma.pay_transparency_calculated_data.findMany).toHaveBeenCalledTimes(1);
      expect(reportAndCalculations.report).toEqual(mockReportInDB);
      expect(Object.keys(reportAndCalculations.calculations).length).toEqual(mockCalculatedDatasInDB.length);
    })
  })
  describe("when the user isn't associated with a company", () => {
    it("throws an error", async () => {
      ...
      (utils.getSessionUser as jest.Mock).mockReturnValue(mockUserInfo);
      (prisma.pay_transparency_company.findFirst as jest.Mock).mockResolvedValue(null);

      await expect(reportService.getReportAndCalculations(mockReq, mockReportId)).rejects.toThrow();
      expect(prisma.pay_transparency_company.findFirst).toHaveBeenCalledTimes(1);
    })
  })
  describe("when an invalid report id is provided", () => {
    it("throws an error", async () => {
      ...
      (utils.getSessionUser as jest.Mock).mockReturnValue(mockUserInfo);
      (prisma.pay_transparency_company.findFirst as jest.Mock).mockResolvedValue(mockCompanyInDB);
      (prisma.pay_transparency_report.findFirst as jest.Mock).mockResolvedValue(null);
