...
comment on column reports_view.is_unlocked is 'Indicates if the report is unlocked for editing.';
...
comment on column reports_view.admin_user_id is 'Admin users can lock a report which updates `report_unlock_date` and `admin_modified_date`. They can also look at reports which updates `admin_last_access_date`.';
comment on column reports_view.admin_modified_date is 'The time when an admin user locked or unlocked a report.';
comment on column reports_view.admin_last_access_date is 'The time when an admin user last looked at this report.';
...
comment on column reports_view.calculated_data is 'This JSON column contains the all calculated data for the report instead of being in a separate view due to a limitation of the Database ORM.';